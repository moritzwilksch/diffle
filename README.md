...
Review export is available when the comparison matches an open PR's committed diff. Unpushed commits, worktree comparisons, and different base comparisons can show repository or PR information without enabling export. Stale threads are skipped, and so are threads GitHub would not show: on a file outside the diff, or on a line beyond the three lines of context GitHub renders around a change. Threads on a whole file become GitHub file-level comments.
