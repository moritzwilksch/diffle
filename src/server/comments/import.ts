import type { CommentAuthor, CommentThread, Side, ThreadCreate } from '../../shared/protocol.js';

/** A payload that is not valid JSON or not a ThreadCreate. The caller's fault. */
export class ImportError extends Error {}

/** Everything `POST /api/threads` and `--comment` accept: 1 MiB, like the HTTP body limit. */
const MAX_PAYLOAD_BYTES = 1024 * 1024;

/**
 * Parses `--comment` / `POST /api/threads` payloads: one object, or an array of
 * them, as a JSON string or an already-parsed value. Throws ImportError on any
 * malformed entry, naming its index.
 */
...
function toThreadCreate(entry: unknown, where: string): ThreadCreate {
  ...
  if (x.author != null && x.author !== 'human' && x.author !== 'agent') return fail("author must be 'human' or 'agent'");
  if (x.authorName != null && typeof x.authorName !== 'string') return fail('authorName must be a string');
  ...
  if (x.author != null) t.author = x.author as CommentAuthor;
  if (x.authorName != null) t.authorName = x.authorName as string;
