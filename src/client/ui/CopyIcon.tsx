...
export function CopyIcon({ done, children, size = '0.875rem' }: { done: boolean; children: ReactNode; size?: string }) {
  ...
  return (
    <span className="relative inline-block size-3.5" aria-hidden="true">
      <span className={twMerge(transition, done && 'opacity-0 [transform:scale(0.4)]')}>{children}</span>
      <span
        className={twMerge(
          transition,
          done ? 'opacity-100 [transform:scale(1)_rotate(0)]' : 'opacity-0 [transform:scale(0.4)_rotate(-30deg)]',
        )}
      >
        <Check size={size} />
      </span>
